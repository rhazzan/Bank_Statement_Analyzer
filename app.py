...
====================
Production-ready Nigerian bank statement analyser — Streamlit app.

ROOT CAUSE OF THE DATASET TAB BUG (and all the other bugs)
------------------------------------------------------------
1. `df` was created INSIDE `with tab1:` → it only existed in that local
   scope.  When Streamlit rendered `tab2` it raised NameError because `df`
   was never stored anywhere accessible outside tab1's block.

2. `uploaded_file` was passed directly to `pd.ExcelWriter(uploaded_file …)`
   which tried to *overwrite* the Streamlit UploadedFile object — that is
   not a writable file path and caused cryptic IO errors.

3. Tabs were created TWICE (before and after the button click), which made
   Streamlit create two independent sets of tab widgets.  The second set
   (inside the if-button block) was never rendered to the user.

4. `from Dashboardcode import run_dashboard` was at an indentation level
   that placed it inside the `except` clause of the savings try-block,
   causing an ImportError to silently swallow the dashboard render.

5. Date / numeric conversion had no `errors='coerce'` protection on the
   main transactions sheet, so a single bad value crashed the whole pipeline.

HOW THE REFACTOR FIXES EVERYTHING
----------------------------------
* `st.session_state` is used as the single source of truth for the
  processed dataframe.  Every tab reads from session_state — no scope
  issues are possible.
* File bytes are read once into `BytesIO`; a second `BytesIO` buffer is
  used for the Excel report so the uploaded file is never mutated.
* Tabs are created exactly once, unconditionally.
* Every risky operation is wrapped in try/except with `st.error()` /
  `st.warning()` feedback.
* All helper functions are pure (no side-effects on globals).
* `@st.cache_data` is used on the heavy loading + cleaning step so
  re-runs caused by widget interaction don't re-process the file.
"""
